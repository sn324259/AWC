	...
	for i,val := range FormIndex{
		...
		if oemFormNo==valIndex.FAA_FormNumber{
			...
			if part_type=="Tier-2"{
				...
				queryArgs:=util.ToChaincodeArgs([]string{f,tier1FormNo,"Tier-1"})
				...
				queryArgs:=util.ToChaincodeArgs([]string{f,tier2FormNo,"Tier-2"})
			...
			if part_type=="Tier-3"{
				...
				queryArgs:=util.ToChaincodeArgs([]string{f,tier3FormNo,"Tier-3"})
