	...
	for i,val := range FormIndex{
		...
		if oemFormNo==valIndex.FAA_FormNumber{
			...
			if part_type=="Tier-2"{
				...
				tier1_to_oem_Shipment_id_as_byte,err := stub.QueryChaincode(chaincodeURL,f,["Tier-1"])
				...
				tier2_to_tier1_Shipment_id_as_byte,err := stub.QueryChaincode(chaincodeURL,f,["Tier-2"])
			...
			if part_type=="Tier-3"{
				...
				tier3_to_tier2_Shipment_id_as_byte,err := stub.QueryChaincode(chaincodeURL,f,["Tier-3"])
